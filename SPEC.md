...
PNG 劝阻         PNG 是无损格式，体积基本压不下来。要变小请选 JPG 或 WebP。
CTA 默认         压缩这 {n} 张
CTA 处理中       处理中 {i} / {n}
CTA 完成         再压一次
...
| 输出比原图还大 | 返回原文件字节，标 `undershot`，文案「这张已经压到底了」 |
| 压不到目标体积 | 标 `undershot`，文案「质量已到下限，只压到 {x}」。绝不为了达标牺牲观感 |
