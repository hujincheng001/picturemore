...
export const COPY = {
   ...
   * 输出格式选 PNG 时的提示。
   *
   * ⚠️ **原来的文案是错的，2026-09-22 实测后改掉。**
   *
   * 原文：「PNG 是无损格式，体积基本压不下来。要变小请选 JPG 或 WebP。」
   * 那是按直觉写的，没人拿实测核过。实测（65% 档，全部 fixture 转 PNG）：
   *
   *   flat-solid.png    44KB ->  3KB   -93%  达标
   *   alpha-cutout.png  31KB ->  2KB   -95%  达标
   *   oriented-6.jpg     7KB ->  2KB   -78%  达标   ← 输入是 JPG 也压得动
   *   noise-hi.jpg    11.9MB -> 交原图（PNG 更大）
   *   sample.webp      3.1MB -> 交原图
   *
   * 分界线是**颜色数**不是输入格式：≤256 色（纯色 / 截图 / 图标 / 线条图）
   * 走调色板量化，能压 80% 以上；照片几十万色，无损格式无能为力。
   *
   * 而「截图和纯色图」恰恰是「发不出去的图」里很大的一类，
   * 原话会直接劝退这部分用户。所以改成如实说两边。
   */
  pngDissuade: 'PNG 是无损格式。照片类压不动，截图和纯色图能压很多。',
